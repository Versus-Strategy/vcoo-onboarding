...
def get_provision_token(vcoo_id: str, db: Session = Depends(get_db)):
    ...
    install_cmd = f"curl -sSL https://example.com/install.sh | PROVISION_TOKEN={token} bash -"
...
def register_agent(payload: dict, db: Session = Depends(get_db)):
    ...
    if not v:
        raise HTTPException(status_code=404, detail="VCOO not found")
    # create agent record
    agent = crud.create_agent(db, vcoo_id=vcoo_id, info=json.dumps(info), token_jti=None)
